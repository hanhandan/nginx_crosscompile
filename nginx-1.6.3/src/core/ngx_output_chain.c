...
ngx_output_chain(ngx_output_chain_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    if (ctx->in == NULL && ctx->busy == NULL) {
        ...
        if (in->next == NULL
            ...
            && !(in->buf->in_file && (int)(in->buf->file_last) >(int)NGX_SENDFILE_LIMIT)
#endif
...
ngx_output_chain_as_is(ngx_output_chain_ctx_t *ctx, ngx_buf_t *buf)
{
    ...
    if (buf->in_file && (int)(buf->file_pos) >= (int)NGX_SENDFILE_LIMIT) {
        sendfile = 0;
    }
    ...
    ngx_chain_t *in)
{
    ...
    while (in) {
        ...
        if (buf->in_file
            && (int)buf->file_pos < (int)NGX_SENDFILE_LIMIT
            && (int)buf->file_last > (int)NGX_SENDFILE_LIMIT)
        {
...
ngx_output_chain_copy_buf(ngx_output_chain_ctx_t *ctx)
{
    ...
    if (src->in_file && src->file_pos >= (int)NGX_SENDFILE_LIMIT) {
        sendfile = 0;
    }
